...
def get_trainer(language):
    """Get trainer for the specified language, loading it dynamically if needed."""
    if language not in trainer_SST_lambda:
        print(f"Loading trainer for language: {language}")
...
def lambda_handler(event, context):
    ...
    if event.get('warmup'):
        print("Warm-up or non-API event received, keeping function warm")
        return {
            'statusCode': 200,
            'headers': {
